...
class CCA(object):
    def __init__(self, key_path="./key.bin", iv_path="./iv.bin", mac_key_path="./mac_key.bin"):
    ...
    def get_mac_from_chunk(self, image):
        metadata = image.info
    ...
    def decrypt_schema(self, encrypted):
        decryptor = self.cipher.decryptor()
        plaintext = decryptor.update(encrypted) + decryptor.finalize()
        return plaintext

    def generate_image_mac(self, image_data):
        # 生成图像内容的 MAC
    ...
    def process_image(self, input_path, output_path, mode='encrypt'):
        ...
        if mode == 'encrypt':
            ...
            image_mac = self.generate_image_mac(image_data)
            ...
            # 提取出图像中的 MAC
            image_mac = self.get_mac_from_chunk(img)

            # 对图片的 RGB 内容部分重新生成 MAC
            image_data = img.tobytes()
            rgb_mac = self.generate_image_mac(image_data)
