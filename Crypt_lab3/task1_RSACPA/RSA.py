...
class RSAENCODE(object):
    def __init__(self, private_path=r".\private.pem", public_path=r".\public.pem",
                 key_path=r".\key.bin", iv_path=r".\iv.bin"):
        if not os.path.exists(private_path) or not os.path.exists(public_path):
            # 没有私钥或公钥，生成一个并写到文件中
            self.private_rsa = RSA.generate(2048, Random.new().read)
            self.public_rsa = self.private_rsa.publickey()  # RsaKey格式，不能写到文件中
            self.private_byte = self.private_rsa.exportKey()
            self.public_byte = self.public_rsa.exportKey()
            print(self.private_rsa)
            print(self.public_rsa)
            with open(private_path, "wb") as f:
                f.write(self.private_byte)
            with open(public_path, "wb") as f:
                f.write(self.public_byte)
        else:
            with open(private_path, "rb") as f:
                self.private_byte = f.read()
            with open(public_path, "rb") as f:
                self.public_byte = f.read()
            self.private_rsa = RSA.importKey(self.private_byte)
            self.public_rsa = RSA.importKey(self.public_byte)
    ...
    def rsa_encode(self, plain_path, cipher_path):
        with open(plain_path, "rb") as f:
            plain = f.read()
        rsa = PKCS1_cipher.new(self.public_rsa)
        cipher = rsa.encrypt(plain)
        with open(cipher_path, "wb") as f:
            f.write(cipher)

    def rsa_decode(self, cipher_path, decode_path):
        with open(cipher_path, "rb") as f:
            cipher = f.read()
        rsa = PKCS1_cipher.new(self.private_rsa)
        plain = rsa.decrypt(cipher, 0)
