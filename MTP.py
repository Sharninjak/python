# 多次使用相同的密钥加密不同明文在窃听攻击下是不安全的。当加密次数足够多时，获得足够多的密文，根据明文特点足以破解。
# 请对以下使用相同的密钥加密的11个十六进制编码密文进行分析，并破解最后一个密文。
# 明文是普通的ASCII编码英文，有空格，2个16进制数字对应一个英文字符。
# 提示：将密文进行异或运算将得到明文的异或结果。考虑在明文的一个空格与一个英文字符[a-zA-Z]异或会得到什么样的密文异或结果。
# 本题给出的密文“足够多”，多到对应明文中的空格出现在了各个位置。
ciphertexts = [
...
def letter_position(s):
    ...
    for idx in range(len(s)):
        # 考虑到空格异或为0的情况可能较多
        if s[idx] in 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz' or s[idx] == chr(0):
            position.append(idx)
    return position
...
print(result)
print(strxor(key_hex, ciphertexts[9]))
'''
#打印所有密文对应的明文
for j in range(11):
