ciphertexts = [
    "",
    "234c02ecbbfbafa3ed18510abd11fa724fcda2018a1a8342cf064bbde548b12b07df44ba7191d9606ef4081ffde5ad46a5069d9f7f543bedb9c861bf29c7e205132eda9382b0bc2c5c4b45f919cf3a9f1cb74151f6d551f4480c82b2cb24cc5b028aa76eb7b4ab24171ab3cdadb8356f",
    "32510ba9a7b2bba9b8005d43a304b5714cc0bb0c8a34884dd91304b8ad40b62b07df44ba6e9d8a2368e51d04e0e7b207b70b9b8261112bacb6c866a232dfe257527dc29398f5f3251a0d47e503c66e935de81230b59b7afb5f41afa8d661cb",
    "",
    "3f561ba9adb4b6ebec54424ba317b564418fac0dd35f8c08d31a1fe9e24fe56808c213f17c81d9607cee021dafe1e001b21ade877a5e68bea88d61b93ac5ee0d562e8e9582f5ef375f0a4ae20ed86e935de81230b59b73fb4302cd95d770c65b40aaa065f2a5e33a5a0bb5dcaba43722130f042f8ec85b7c2070",
    "",
    "",
    "315c4eeaa8b5f8bffd11155ea506b56041c6a00c8a08854dd21a4bbde54ce56801d943ba708b8a3574f40c00fff9e00fa1439fd0654327a3bfc860b92f89ee04132ecb9298f5fd2d5e4b45e40ecc3b9d59e9417df7c95bba410e9aa2ca24c5474da2f276baa3ac325918b2daada43d6712150441c2e04f6565517f317da9d3",
    "",
    "466d06ece998b7a2fb1d464fed2ced7641ddaa3cc31c9941cf110abbf409ed39598005b3399ccfafb61d0315fca0a314be138a9f32503bedac8067f03adbf3575c3b8edc9ba7f537530541ab0f9f3cd04ff50d66f1d559ba520e89a2cb2a83",
    "32510ba9babebbbefd001547a810e67149caee11d945cd7fc81a05e9f85aac650e9052ba6a8cd8257bf14d13e6f0a803b54fde9e77472dbff89d71b57bddef121336cb85ccb8f3315f4b52e301d16e9f52f904"
]
NUM_CIPHER = len(ciphertexts)  # NUM_CIPHER=11
TARGET = ciphertexts[10]


def to_ascii(hex_string):
    ascii_string = ''.join([chr(int(hex_string[i:i + 2], 16)) for i in range(0, len(hex_string), 2)])
    return ascii_string


# 将两个字符串的16进制形式串进行异或
def str_xor(a, b):
    a = to_ascii(a)
    b = to_ascii(b)
    if len(a) > len(b):
        # 形成二元组，异或,返回新字符串
        return "".join([chr(ord(x) ^ ord(y)) for (x, y) in zip(a[:len(b)], b)])
    else:
        return "".join([chr(ord(x) ^ ord(y)) for (x, y) in zip(a, b[:len(a)])])


# 将字符串中的非字母改为*号
def xor_change(a):
    length = len(a)
    for i in range(length):
        if a[i] in 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz':
            continue
        else:
            a_list = list(a)
            a_list[i] = '*'
            a = ''.join(a_list)
    # print(a)
    return a


# 同一位置出现重复字母认为此处的目标明文就是该字母
def xor_solution1(cipher_xor):
    length_10 = len(cipher_xor[9])
    # print(length_10)
    plaintext = ''
    for i in range(length_10):  # i: xth character 0~length_10-1
        temp = ' '
        for j in range(NUM_CIPHER-1):  # j: xth string 0~9
            if cipher_xor[j][i] == '*':
                pass
            # elif cipher_xor[j][i] != '*' and temp == ' ':
            elif temp == ' ':
                temp = cipher_xor[j][i]
            # elif cipher_xor[j][i] != '*' and cipher_xor[j][i] == temp:
            elif cipher_xor[j][i] == temp:
                plaintext = plaintext+temp.swapcase()
                break
            # elif cipher_xor[j][i] != '*' and cipher_xor[i][j] != temp:
            else:
                temp = ' '
            if j == 9:
                plaintext = plaintext+temp.swapcase()
            # print(plaintext)
    return plaintext


# 同一位置出现不同字母认为此处目标明文为空格
def xor_solution2(cipher_xor):
    length_10 = len(cipher_xor[9])
    # print(length_10)
    plaintext = ''
    for i in range(length_10):  # i: xth character 0~length_10-1
        temp = ' '
        for j in range(NUM_CIPHER-1):  # j: xth string 0~9
            if cipher_xor[j][i] == '*':
                pass
            # elif cipher_xor[j][i] != '*' and temp == ' ':
            elif temp == ' ':
                temp = cipher_xor[j][i]
            # elif cipher_xor[j][i] != '*' and cipher_xor[j][i] == temp:
            elif cipher_xor[j][i] != temp:
                temp = ' '
                plaintext = plaintext+temp.swapcase()
                break
            # elif cipher_xor[j][i] != '*' and cipher_xor[i][j] != temp:
            if j == 9:
                plaintext = plaintext+temp.swapcase()
            # print(plaintext)
    return plaintext


def solution1():
    # list cipher_xor, store ciphertext0~9 xor ciphertext10
    cipher_xor = []
    for i in range(NUM_CIPHER-1):
        # print('ciphertext', i, '^ target')
        cipher_xor.append(xor_change(str_xor(ciphertexts[i], ciphertexts[10])))
    print(cipher_xor)
    '''
    ['**EC**C***T**S***EN**XE*HT******GQ*A****A*O********N**E*A*S*L**EF***O*O**ET***B**CT',
     '***E*E****DM******L*S*N***NT***N*O*****E**E****E*IC****RAU**R*******N*O*******T*NNE',
     '********E*H***S***U*SqE****QU**N*O****R***P******DE**V**NU**I**EAK**TM**EF*********',
     '**********T***S***D***Dw**NAU******N******O*I*****I***E*O******EG******R*I****T***E',
     '*******U*TW***S**EB***Aw******I**RAK***E**O*I*H**U****E*P***A***E*E*NM***A*********',
     '***R*E***TT**S****SI*******T*****H***T**********R*I**V**E*S*E***T*E*A**R*R**A*O**C*',
     '***R*E***TT**S****SI*******O*****Y****N*******I****N***Rg***R***N*E*OM********EO***',
     '**EC**C*******S***N*SMH***NT**I**I****R***A***H***AN****GU**TT******TM********U***E',
     '*HMP**********ZAG*N**CP**********EAS*****M*C*****ET***IRN***L*H*****C****ET********',
     't**ES*****S*E*****D**YT****R*SA*W*W*S*****N**P****T*E**RT**EA**EO*EYW****N*H*NRO***']
    '''
    plaintext1 = xor_solution1(cipher_xor)
    print(plaintext1)
    plaintext2 = xor_solution2(cipher_xor)
    print(plaintext2)


# def guess_key():
#     plaintext = 'The secret message is: When using a stream cipher, never use the key more than once'
#     plaintext_hex = ''.join(hex(ord(char))[2:] for char in plaintext)
#     print(plaintext_hex)
#     key = str_xor(plaintext_hex, TARGET)
#     print(key)
#     return key
#
#
# def verify_key():
#     key = guess_key()
#     print(to_ascii(key))

if __name__ == "__main__":
    solution1()
    # verify_key()
